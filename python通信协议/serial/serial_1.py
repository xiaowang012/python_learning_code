...
ser.write("hello") #向端口写数据，有一个转码问题，decode('utf-8')
ser.close() #关闭端口
data = ser.read(20) #是读20个字符
data = ser.readline() #是读一行，以/n结束，要是没有/n就一直读，阻塞。
data = ser.readlines()和ser.xreadlines()#都需要设置超时时间
ser.baudrate = 9600 #设置波特率
ser.isOpen() #看看这个串口是否已经被打开.布尔值
...
def serial_1(serial_com_value,bps_value,timeout):
    '''serial login'''
    #指定com值(1,2,3,4...)，波特率(9600,115200,38400),超时设置的ser对象
    try:
        ser=serial.Serial(serial_com_value,bps_value,timeout)
    except:
        print('open serial error')
    else:
        #向串口写数据，需要转码，decode('UTF-8')或者在开头全局申明utf-8
        ser.write(b'\n')
        sleep(1)
